...
function render(items) {
    ...
    items.map(item => {
        ...
        reviews[0].innerHTML = `<strong>${item.orderInfo.reviews}%</strong> <br>Positive reviews <br>Above avarage`
            // btn[0].addEventListener("click", (e) => e.stopPropagation())
...
function filtreredData(device) {
    ...
    let resDevice = filterDevice.find(fiterItem => device.display > fiterItem.from && device.display < fiterItem.to)
    let resOs = filterOs.find(fiterItem => fiterItem.from == device.os)
        //let allKeys =filter.keys()
    return !!resOs && !!resDevice
}